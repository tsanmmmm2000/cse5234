...
public class IWorkWithJSPServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String personalizedGreeting = String.format("Hello %s!", name);
		int counter = (request.getSession().getAttribute("counter") != null) 
				? (int) request.getSession().getAttribute("counter") + 1
				: 1;
		personalizedGreeting += "\nYou visited " + counter + " times!!";		
		request.getSession().setAttribute("counter", counter);
		request.setAttribute("personalizedGreeting", personalizedGreeting);	
